...
namespace netca.Api.Middlewares
{
    ...
    public class OverrideResponseHandlerMiddleware
    {
        ...
        /// <returns></returns>
        public OverrideResponseHandlerMiddleware(RequestDelegate next, IRedisService redisService, AppSetting appSetting, ILogger<OverrideResponseHandlerMiddleware> logger)
        {
            _next = next;
        ...
        private Policy IsCache(string policy)
        {
            ...
            return policyList.Count.Equals(0) ? null : policyList.SingleOrDefault(x => x.Name.ToLower().Equals(policy.ToLower()));
        }
    }
